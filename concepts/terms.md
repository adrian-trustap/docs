...
1. Create a user with the appropriate scopes. 
    * [Create face-to-face user](../../docs/guides/transactions/face-to-face/face-to-face-create-full-user.md). 
    * [Create online user](../../docs/guides/transactions/online/online-create-full-user.md).

2. Add `Trustap-User` to the header of your API call with your user's `id`.

```CURL {% title="Example of Trustap-User being used when making a complaint" %}
...
## Webhooks
Trustap sends webhook calls to configured webhook endpoint using basic authentication (username + password). Every request contains information about a [resource](./webhooks.md) and the code which triggered this webhook call. Usually, one webhook handler is enough, but could be configured to use multiple as well. This is useful when testing on a Stage environment. 
...
See our [Merchant of Record](./merchant-of-record.md) guide for more information.