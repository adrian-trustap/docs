...
| `<REDIRECT_URI>` | The callback URL used after [full Trustap account creation](../guides/transactions/online/online-create-full-user.md). This must match the redirect URI used in your flow. Trustap supports wildcard URIs. For example, if you provide `https://www.sams-bikes.com/*`, you can redirect to `https://www.sams-bikes.com/login_path/`.|
| `<CALLBACK_PAYMENT>` | The URL to which users are redirected after [completing payment](../guides/transactions/online/online-int-path.md#step-5-buyer-transfers-funds). Trustap also supports wildcards here. For example, `https://www.sams-bikes.com/payment/*`. |
